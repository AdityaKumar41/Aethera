...
import { TrendingUp, Home, Key, Calendar, Shield, Zap } from "lucide-react"

const revenueCategories = [
  { name: "Energy Sales", icon: Zap, color: "#f59e0b" },
  { name: "Grid Services", icon: TrendingUp, color: "#3b82f6" },
  { name: "Carbon Credits", icon: Shield, color: "#10b981" },
  { name: "Maintenance", icon: Home, color: "#8b5cf6" },
]
...
const initialData = revenueCategories.map((cat) => ({
  ...cat,
  value: 15000,
}))
  ...
  return (
    ...
    >
      <div className="mb-8 flex items-center justify-between">
        <div>
          <h3 className="text-lg font-semibold text-slate-900">Solar Revenue</h3>
        ...
        {data.map((item, index) => {
          ...
          return (
            <motion.div
              ...
              className={`flex items-center gap-3 p-2 rounded-xl transition-colors ${index === activeIndex ? "bg-slate-50" : ""
                }`}
            >
              <div
