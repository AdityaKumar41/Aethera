...
export function PortfolioSection() {
    ...
    // Derived Top Assets
    const topAssets = (portfolio?.investments || []).map((i: any) => ({
        id: i.id,
        name: i.project?.name || "Solar Project",
        value: Number(i.amount),
        yield: i.project?.expectedYield || 10,
        change: 2.4, // Mock change for now
        color: i.project?.status === "ACTIVE" ? "from-emerald-500 to-green-600" : "from-blue-500 to-indigo-600",
        pending: i.status === "PENDING" || i.status === "PENDING_ONCHAIN",
    })).sort((a, b) => b.value - a.value).slice(0, 5);

    // Derived Allocation
    const allocationData = (portfolio?.investments || []).reduce((acc: any[], current: any) => {
    ...
    const chartData = ["Jul", "Aug", "Sep", "Oct", "Nov", "Dec", "Jan"].map((month, idx) => ({
        month,
        ...
        <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <MetricCard
                    ...
                    change={totalYield > 0 ? "+8.3%" : "0%"}
                    changeType={totalYield > 0 ? "positive" : "neutral"}
                    icon={Coins}
                    delay={1}
                    gradient="energy"
                />
