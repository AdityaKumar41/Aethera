...
export function AdminProjectsSection() {
  const [activeTab, setActiveTab] = useState<"projects" | "activation" | "milestones">("projects");
  ...
  const fetchSubmittedMilestones = async () => {
    setLoading(true);
    try {
      // We'll need to add this to adminApi
      // For now, we simulate or use a generic fetch
      const response = await apiRequest<any[]>("/api/milestones/submitted");
      if (response.success) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-muted-foreground">Review submissions and activate funded projects.</p>
        </div>
          ...
          <div className="bg-zinc-100 p-1 rounded-xl flex">
            <button
              ...
              className={cn(
                ...
                activeTab === "projects" ? "bg-white text-purple-600 shadow-sm" : "text-muted-foreground hover:text-foreground"
              )}
            >
              Pending Approval
            </button>
            <button
              ...
              className={cn(
                "px-4 py-2 rounded-lg text-sm font-medium transition-all",
                activeTab === "activation" ? "bg-white text-emerald-600 shadow-sm" : "text-muted-foreground hover:text-foreground"
              )}
            >
              Ready to Activate
            </button>
            <button
              ...
              className={cn(
                "px-4 py-2 rounded-lg text-sm font-medium transition-all",
                activeTab === "milestones" ? "bg-white text-blue-600 shadow-sm" : "text-muted-foreground hover:text-foreground"
              )}
            >
              Milestones
            </button>
      ...
      {loading ? (
        ...
        filteredProjects.length === 0 ? (
          <div className="bg-card border border-border rounded-2xl py-20 text-center">
            ...
            <h3 className="text-lg font-semibold text-foreground">No projects ready for activation</h3>
            <p className="text-muted-foreground max-w-[280px] mx-auto">
        ...
        <MilestoneList milestones={milestones} onVerify={handleVerifyMilestone} processingId={processingId} />
      )}
    </div>
  );
}
...
function ProjectCard({ project, onApprove, onReject, onActivate, processingId, type }: any) {
    return (
                ...
                <div className="flex-1 space-y-4">
                    <div className="flex items-start justify-between gap-4">
                        ...
                        <div className="flex items-center gap-2 mb-1">
                            <span className={cn(
                                ...
                                type === 'PENDING' ? "bg-purple-50 text-purple-700" : "bg-emerald-50 text-emerald-700"
                            )}>
                    ...
                    <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 pt-2">
                        ...
                        <div className="p-3 bg-zinc-50 rounded-xl">
                            {type === 'FUNDED' ? (
                                <>
                                <p className="text-[10px] text-muted-foreground uppercase font-semibold mb-1">Raised</p>
                                <p className="text-sm font-bold text-emerald-600">${Number(project.fundingRaised).toLocaleString()}</p>
                                </>
                            ) : (
                                <>
                ...
                {type === 'PENDING' ? (
                    ...
                    <>
                        {project._count?.iotDevices > 0 ? (
                            ...
                            >
                                {processingId === project.id ? <Loader2 className="w-4 h-4 animate-spin" /> : <Zap className="w-4 h-4" />}
                                Activate Project
                            </button>
                        ) : (
                            <div className="text-center space-y-2">
                                ...
                                <p className="text-[10px] text-muted-foreground leading-tight">
                                    Installer must connect a device before funds can be released.
                                </p>
                            </div>
                        )}
                    </>
...
function MilestoneList({ milestones, onVerify, processingId }: any) {
    if (milestones.length === 0) {
        return (
          <div className="bg-card border border-border rounded-2xl py-20 text-center">
            <Clock className="w-12 h-12 text-zinc-200 mx-auto mb-4" />
            <h3 className="text-lg font-semibold text-foreground">No pending milestones</h3>
            <p className="text-muted-foreground max-w-[280px] mx-auto">
        ...
        {milestones.map((milestone: any) => (
            ...
            <div className="flex flex-col lg:flex-row gap-6">
                ...
                <div className="flex items-center gap-4 text-xs font-semibold text-zinc-500">
                    <span>Release Amount: ${Number(milestone.releaseAmount).toLocaleString()}</span>
                ...
                <button className="w-full py-3 px-4 bg-white border border-red-200 text-red-600 rounded-xl font-semibold hover:bg-red-50 transition-all flex items-center justify-center gap-2">
                    <XCircle className="w-4 h-4" />
                    Reject
                </button>
                </div>
